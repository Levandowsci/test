def calculate_profit(initial_amount, monthly_deposit, annual_interest_rate, deposit_period):
    ...
    for month in range(1, deposit_period + 1):
        ...
        monthly_interest = total_amount * monthly_interest_rate  # Рассчитываем проценты за текущий месяц
        total_amount += monthly_interest  # Прибавляем проценты к общей сумме

        print(f"Месяц {month}: Общая сумма на счету - {total_amount:.2f}, Прибыль в этом месяце - {monthly_interest:.2f}")
...
total_amount = calculate_profit(initial_amount, monthly_deposit, annual_interest_rate, deposit_period)
print(f"Общая сумма на счету после {deposit_period} месяцев составит: {total_amount:.2f}")
